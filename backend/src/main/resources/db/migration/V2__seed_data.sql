...
INSERT INTO dishes(name, price_cents, description, is_available, max_quantity_per_order, created_at, updated_at)
SELECT '宫保鸡丁', 3200, '经典下饭菜', 1, 5, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM dishes WHERE name = '宫保鸡丁');

INSERT INTO dishes(name, price_cents, description, is_available, max_quantity_per_order, created_at, updated_at)
SELECT '鱼香肉丝', 2800, '微辣开胃', 1, 8, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM dishes WHERE name = '鱼香肉丝');

INSERT INTO dishes(name, price_cents, description, is_available, max_quantity_per_order, created_at, updated_at)
SELECT '青椒土豆丝', 1800, '家常清爽', 1, 10, NOW(), NOW()
